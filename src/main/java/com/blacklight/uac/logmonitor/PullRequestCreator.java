        ...
        try {
            ...
            if (runLocal) {
                ...
                if (pushExitCode != 0) {
                    ...
                    // 5. Create PR via gh CLI (uses gh auth / SSH – no GITHUB_TOKEN needed)
                    System.out.println(YELLOW + "  → Creating PR via gh CLI..." + RESET);
                    try {
                        ProcessBuilder prPb = new ProcessBuilder(
                            "gh", "pr", "create",
                            "--title", "fix: Automated fix for " + fix.getFixDescription(),
                            "--body",  "Automated fix generated by UAC (Universal Autonomous Core).\n\nBranch: " + branchName,
                            "--head",  branchName,
                            "--base",  "main"
                        );
                        prPb.directory(repoDir);
                        prPb.redirectErrorStream(false);
                        Process prProc = prPb.start();
                        String prUrl = new String(prProc.getInputStream().readAllBytes()).trim();
                        prProc.getErrorStream().transferTo(java.io.OutputStream.nullOutputStream());
                        int prExit = prProc.waitFor();
                        if (prExit == 0 && prUrl.contains("github.com")) {
                            System.out.println(GREEN + "  ✓ PR created: " + prUrl + RESET);
                        } else {
                            System.out.println(YELLOW + "  → gh pr create did not succeed (gh may not be installed/authed)" + RESET);
                        }
                    } catch (Exception prEx) {
                        System.out.println(YELLOW + "  → gh CLI not available: " + prEx.getMessage() + RESET);
                    }
                }
            }
